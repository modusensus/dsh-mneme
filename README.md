...
<p align="center">
  ...
  <a href="https://github.com/modusensus/dsh-mneme"><img src="https://img.shields.io/badge/tests-404%20passed-success" alt="tests"></a>
</p>
...
| **v0.2.9** | 社区建议四连 | ✅ 已完成（冲突冻结配置：不自动裁决、待人工确认，355 测试） | — |
| **v0.2.10** | 镜像 digest 基线 | ✅ 已完成（last-rendered digest 修复未触碰镜像被误判为人工编辑，363 测试） | — |
| **v0.2.11** | 审计复验修复 | ✅ 已完成（F-NEW-01 启动回灌复用 digest + F-NEW-02 commit/mirror 分离，373 测试） | — |
| **v0.2.12** | 轻量结构化 | 实体注释标记（零 schema 变更，验证可行性） | 2-3 天 |
| **v0.3.0** | 记忆基因 | ✅ 已完成（entities/attrs/relations 三表 + valid_until 时间轴 + LLM 抽取 + entity:/attr: 搜索 + autoDream supersedes，404 测试） | — |
...
|---------|------------|-----------|----------|
| **v0.2.1** | Reflection update | ✅ Done (update decision + failure_memories, 404 tests) | — |
| **v0.2.2** | Pipeline completion | ✅ Done (specialized tests + review fixes, 404 tests) | — |
| **v0.2.4** | Security hardening | ✅ Done (API auth apiToken + apiKey masking + timing-safe, 404 tests) | — |
| **v0.2.5** | Security audit fixes | ✅ Done (CAS + transactional apply + reconcile audit + reranker opt-in + archive restore, 404 tests) | — |
| **v0.2.6** | Community contribution | ✅ Done (PR #2 embedded panel UI fix, 404 tests) | — |
| **v0.2.7** | Honest audit | ✅ Done (F-03: noop/degraded states, no false ok on empty runs, 404 tests) | — |
| **v0.2.8** | Community suggestions ×3 | ✅ Done (recall-layer receipt + policy_epoch + per-record receipt chain, 404 tests) | — |
| **v0.2.9** | Community suggestions ×4 | ✅ Done (conflict freeze config: no auto-adjudication, manual review, 404 tests) | — |
| **v0.2.10** | Mirror digest baseline | ✅ Done (last-rendered digest fixes untouched-mirror misread as human edit, 404 tests) | — |
| **v0.2.11** | Audit re-verification fixes | ✅ Done (F-NEW-01 startup backfill reuses digest + F-NEW-02 commit/mirror separation, 404 tests) | — |
| **v0.2.12** | Lightweight structure | Entity comment markers (zero schema change, feasibility check) | 2-3 days |
| **v0.3.0** | Memory genome | ✅ Done (entities/attrs/relations tables + valid_until timeline + LLM extraction + entity:/attr: search + autoDream supersedes, 404 tests) | — |
