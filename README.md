...
<p align="center">
  ...
  <a href="https://github.com/modusensus/dsh-mneme"><img src="https://img.shields.io/badge/tests-373%20passed-success" alt="tests"></a>
</p>
...
|---------|------------|-----------|----------|
| **v0.2.1** | Reflection update | ✅ Done (update decision + failure_memories, 373 tests) | — |
| **v0.2.2** | Pipeline completion | ✅ Done (specialized tests + review fixes, 373 tests) | — |
| **v0.2.4** | Security hardening | ✅ Done (API auth apiToken + apiKey masking + timing-safe, 373 tests) | — |
| **v0.2.5** | Security audit fixes | ✅ Done (CAS + transactional apply + reconcile audit + reranker opt-in + archive restore, 373 tests) | — |
| **v0.2.6** | Community contribution | ✅ Done (PR #2 embedded panel UI fix, 373 tests) | — |
| **v0.2.7** | Honest audit | ✅ Done (F-03: noop/degraded states, no false ok on empty runs, 373 tests) | — |
| **v0.2.8** | Community suggestions ×3 | ✅ Done (recall-layer receipt + policy_epoch + per-record receipt chain, 373 tests) | — |
| **v0.2.9** | Community suggestions ×4 | ✅ Done (conflict freeze config: no auto-adjudication, manual review, 373 tests) | — |
| **v0.2.10** | Mirror digest baseline | ✅ Done (last-rendered digest fixes untouched-mirror misread as human edit, 373 tests) | — |
| **v0.2.11** | Audit re-verification fixes | ✅ Done (F-NEW-01 startup backfill reuses digest + F-NEW-02 commit/mirror separation, 373 tests) | — |
