// 记忆语言切换（memory.language，src/config.js）：生成记忆、注入标题与后台
// LLM 提示词所用语言，'zh'（默认，上游行为不变）/ 'en'。启动时由 index.js
// 读入一次；所有消费方在调用时经 STR.<key>[lang()] 取值，切换无需重新
// import，也不改变任何调用方结构。
export const LANGUAGES = ["zh", "en"];

let active = "zh";

export function setMnemeLanguage(value) {
  if (value === "zh" || value === "en") active = value;
}

export function getMnemeLanguage() {
  return active;
}

/** 当前语言键（"zh" | "en"）；各语言对用该键索引。 */
...
export const STR = {
  ...
  // --- summarize.js：蒸馏转录行标签（进 LLM 上下文） -------------------------
  ...
  // --- mirror.js：镜像文件标签（渲染随语言；解析两种语言都认） -------------------
  mirrorLabel: {
