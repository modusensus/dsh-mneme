...
- **better-sidebar 0.19.x registerTab 兼容（#145）**：better-sidebar tab 注册先携带 `kind`（`kind=id`）以兼容 0.19.x 原生右栏桥接链路；若宿主 registerTab 不接受该字段则自动回退到旧描述符重试，确保旧版行为不回归。同时将 optional peer 范围放宽为 `^0.18.0 || ^0.19.0`。
