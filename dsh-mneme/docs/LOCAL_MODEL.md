...
| GPU（可选） | 显存 ≥ 2GB | 开启 `embedDevice: "gpu"`（onnxruntime 需要额外安装 cuda 后端） |
...
```bash
rm -rf ~/.cache/huggingface/hub/models--Xenova--bge-small-zh-v1.5
```
...
- id: dsh-mneme
  ...
  config:
    ...
    embedDevice: cpu            # cpu | wasm | gpu（仅 local 生效）
    embedBatchSize: 8           # 分批嵌入条数，内存紧张调小
    embedCacheDir: ""           # 模型缓存目录（local 生效，空=HF 默认缓存）
...
|------|--------------|--------|--------|
| `embedModel` | HF 模型 id（如 `Xenova/bge-small-zh-v1.5`） | Ollama 模型名（如 `nomic-embed-text`） | API 模型名（如 `text-embedding-3-small`） |
...
> 注意：切换 `embedModel` 或 `embedDimension` 后，旧向量与新向量维度不同、无法混算，需**重建索引**（复用 v0.1 的 `reindexMissing` 机制）。
